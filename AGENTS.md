...
> **와이어프레임 전 화면(01~16) 1차 구현 완료.** 이어서 작업할 땐 반드시 **`docs/PROGRESS.md`(진행 상태·작업 절차) → `docs/be-api-contract.md`(BE 계약)** 를 먼저 읽고 그 절차/규칙대로 구현·커밋한다. 미구현 BE 조회 API 는 provisional(MSW) + `// TODO(fe-api)` 마커로 처리되어 있다(`product/docs/FE_API_REQUESTS.md` 인덱스).
