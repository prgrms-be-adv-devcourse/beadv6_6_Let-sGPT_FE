# AGENTS.md — Let'sGPT FE 작전 지시서

별도 Spring BE(MSA)의 API를 소비하는 React SPA. 구현은 에이전트가 검증 루프를 돌며 수행한다.
상세 배경/근거는 `docs/frontend-harness-guide.md`, 화면 정의는 `screens/`.
...
- 새 코드는 기존 시드(`features/product`)의 결을 따른다.
