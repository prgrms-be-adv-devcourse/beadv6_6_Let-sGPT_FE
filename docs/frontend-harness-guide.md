...
## 현재 상태 스냅샷 (FE 세션 진입점 — 2026-06-27)
...
- **와이어프레임**: 화면당 `screens/NN-name.png`(레이아웃) + `screens/NN-name.md`(상태·데이터·액션별 API·컴포넌트 매핑, 구현하며 갱신).
- **다음 작업**(§9): 이 가이드 기준 스캐폴딩 — 첫 커밋에 스택 + 검증 인프라(§5) + `AGENTS.md`(§4) + `.claude/settings.json`(BE 스코프 쓰기) + 코드젠 설정.
...
| 층위 | 수단 | 특징 | 우리 도구 |
|---|---|---|---|
| **규칙 기반** | 타입체크 · 린트 · 단위 테스트 | 결정론적, ms~s, 가장 싸다 | TypeScript(strict), Oxlint, Vitest |
...
- [ ] **린트/포맷 on save + on commit** — Oxlint/Oxfmt(또는 Biome), 포맷은 자동 적용
...
1. **스택 확정** — 위 §2 추천안에서 분기점(SPA/Next, Oxlint/Biome 등) 합의 → 확정.
2. **와이어프레임 정의** — 화면·상태·플로우(사람이 정의).
3. **스캐폴딩** — FE 레포 생성 + §5 검증 인프라 + `AGENTS.md`(§4)를 첫 커밋에 구성.
4. **시드 예시 작성** — "올바른" 컴포넌트/훅/테스트/스토리 한 벌을 먼저 심어 패턴을 고정.
5. 이후 기능 구현은 100% 에이전트가 **검증 루프를 돌며** 수행.

> 본 문서는 FE 레포가 생성되면 **그 레포의 `docs/`로 이전**하고, 핵심 운영 규칙은 압축해 **루트 `AGENTS.md`** 로 분리하는 것을 권장한다.
