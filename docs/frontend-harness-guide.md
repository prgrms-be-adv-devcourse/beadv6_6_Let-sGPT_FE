...
- **구현 상태**(§9): 전 화면(01~16) 구현 + 디자인 확정 완료. **실 BE 연동 진행 중** — product 도메인 조회 API(drops·categories·products/drops me)·`sellerName`·이미지 저장(업로드/조회·`imageKeys`)이 BE 구현됨, FE 연동 정합 반영 완료. **남은 BE 갭: wallet 잔액(payment)·판매자 토큰 재발급(member).**
- **인증 모델**: 회원 JWT(전역) + **스토어 범위 판매자 토큰**(활성 스토어 전환 시 재발급, 메모리 보관) — 상세 `docs/auth.md`. 운영 규칙은 `AGENTS.md`(절차)·`docs/design-system.md`(디자인)·`docs/be-api-contract.md`(계약).
- **기록 정책**: product 결정 이력은 **사용자가 직접** `product/docs/DECISIONS.md`에 작성(에이전트는 쓰지 않음).
- **실 BE 콜드 스타트 순서**(MSW→실 연동 전환): ① 게이트웨이 가동 → `/v3/api-docs/all`에 5개 서비스 `/api/v1/...` 경로 확인 → ② `corepack pnpm codegen`(schema.d.ts 실타입) → ③ 초기 시드 데이터 적재(카탈로그가 비지 않도록) → ④ `.env` `VITE_API_MOCKING=disabled`(MSW off) → ⑤ 검증 루프(typecheck·test·스크린샷). 남은 provisional(wallet·seller-token)은 그때까지 MSW 유지.
...
1~5단계(스택 확정 → 와이어프레임 → 스캐폴딩 + §5 검증 인프라 → 시드 예시 → 에이전트 검증 루프 구현)와 **전 화면(01~16) 구현 + 디자인 확정**, **product 도메인 실 BE 연동 정합**(조회 API·`sellerName`·이미지 저장/`imageKeys`)은 완료됐다.

- 운영 규칙은 루트 `AGENTS.md`(작전 지시), 디자인은 `docs/design-system.md`, BE 계약은 `docs/be-api-contract.md`, 인증은 `docs/auth.md` 로 분리·운용 중이다(본 문서는 설계 근거 보존용).
- wallet 잔액(`GET /api/v1/wallet`)·판매자 토큰(`POST /api/v1/seller/token`)은 **BE 구현 확인 완료**(과거 "구현 대기" 표기는 낡았던 것 — 경로/라우트만 어긋났었음). **실 BE 콜드 스타트**(위 스냅샷 순서)로 게이트웨이 가동 → `pnpm codegen` 실타입 교체. 신규 기능은 `AGENTS.md` §3 절차로.
