...
- 남은 것: **wallet 잔액(payment)·판매자 토큰 재발급(member) BE 구현 대기** — provisional + `TODO(fe-api)` 유지, 구현되면 정리. **실 BE 콜드 스타트**(위 스냅샷 순서)로 게이트웨이 가동 → `pnpm codegen` 실타입 교체. 신규 기능은 `AGENTS.md` §3 절차로.
